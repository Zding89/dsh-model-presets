...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function shortcutLabel(index) {
      var apple = typeof navigator !== 'undefined' && /Mac|iPhone|iPad/.test(navigator.platform || navigator.userAgent || '')
      return (apple ? '⌥' : 'Alt+') + String(index + 1)
    }
    ...
    function PresetBar(props) {
      var buttons = props.presets.map(function (preset, index) {
    ...
    function ModelPresetSelect(props) {
      ...
      async function applySelection(selection, closeAfter) {
        ...
        try {
          ...
          setNotice({ error: true, text: props.t('status.switchError', { message: String(error) }) })
          return false
        } finally {
          setBusy(false)
        }
