...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ModelPresetSelect(props) {
      ...
      var shortcutLockRef = React.useRef(false)
      ...
      React.useEffect(function () {
        ...
        var onPresetShortcut = function (event) {
          ...
          if (!preset || !catalog || props.locked || !findModel(catalog, preset.selection)) return
          event.preventDefault()
          event.stopPropagation()
          if (busy || shortcutLockRef.current) return
          shortcutLockRef.current = true
          void applySelection(preset.selection, visible).finally(function () {
            shortcutLockRef.current = false
          })
        }
        window.addEventListener('keydown', onPresetShortcut, true)
        return function () { window.removeEventListener('keydown', onPresetShortcut, true) }
      }, [catalog, busy, presetSnapshot, props.locked, props.sessionId, visible])
